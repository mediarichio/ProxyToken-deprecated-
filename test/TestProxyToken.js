...
describe('ProxyToken', () => {
    ...
    if (runThisTest())
        it('1. verified deployment of the ProxyToken contract by confirming it has a contract address', () => {
            ...
            checkAreNotEqual(ProxyToken.options.address, null);
        });

    if (runThisTest())
        it('2. verified ProxyToken has symbol DYNP', async () => {
            if (!ProxyToken) return;
    ...
    if (runThisTest())
        it('3. verified ProxyToken has 18 decimals', async () => {
            if (!ProxyToken) return;
            result.set(await ProxyToken.methods.decimals().call());
            result.checkIsEqual(WEI_DECIMALS);
        });

    if (runThisTest())
        it('4. verified ProxyToken owner account and total supply initially have 5 billion tokens', async () => {
            if (!ProxyToken) return;
            result.set(await ProxyToken.methods.balanceOf(owner).call());
            result.checkIsEqual(tokensToWei(TOTAL_SUPPLY));
            result.set(await ProxyToken.methods.totalSupply().call());
            result.checkIsEqual(tokensToWei(TOTAL_SUPPLY));
        });

    if (runThisTest())
        it('5. verified ProxyToken non-owner account initially has zero tokens', async () => {
            if (!ProxyToken) return;
            result.set(await ProxyToken.methods.balanceOf(account1).call());
    ...
    if (runThisTest())
        it('6. verified only owner account effectively owns the contract', async () => {
            ...
            await subtest('owner of contract', async () => {
                result.set(await ProxyToken.methods.owner().call());
                result.checkIsEqual(owner);
            }).catch(catcher);

            await subtest('6a. account0 is owner, others are not.', async () => {
                result.set(await ProxyToken.methods.isOwner().call());
                result.checkIsTrue();
                result.set(await ProxyToken.methods.isOwner().call({from: account1}));
                result.checkIsFalse();
                result.set(await ProxyToken.methods.isOwner().call({from: account2}));
                result.checkIsFalse();
            }).catch(catcher);
        });
    ...
    if (runThisTest())
        it('7. verified owner account can change ownership to another account', async () => {
            ...
            await subtest('transfer of contract ownership to account 1', async () => {
                ...
                result.set(await ProxyToken.methods.transferOwnership(account1).send({from: owner, gas: XFEROWNERGAS}));
                result.checkTransactionOk();
                result.set(await ProxyToken.methods.owner().call());
    ...
    if (runThisTest())
        it('8. verified non-owners cannot change ownership', async () => {
            ...
            await subtest('8a. transfer ownership from non-owner account1', async () => {
                result.set(await ProxyToken.methods.registerAccount().send({from: account2}));
                result.checkTransactionOk();
                result.set(await expectFail(ProxyToken.methods.transferOwnership(account2).send({
                    from: account1,
                    gas: XFEROWNERGAS
                })).catch(catcher));
                result.checkDidFail();
            ...
            await subtest('8b. transfer ownership from non-owner account2', async () => {
                result.set(await expectFail(ProxyToken.methods.transferOwnership(owner).send({
                    from: account2,
                    gas: XFEROWNERGAS
                })).catch(catcher));
                result.checkDidFail();
    ...
    if (runThisTest())
        it('9. verified ownership can\'t be renounced', async () => {
            if (!ProxyToken) return;
            result.set(await expectFail(ProxyToken.methods.renounceOwnership().send({from: owner})).catch(catcher));
            result.checkDidFail();
            result.set(await expectFail(ProxyToken.methods.renounceOwnership().send({from: account1})).catch(catcher));
            result.checkDidFail();
        });
    ...
    if (runThisTest())
        it('10. verified ERC20 transfer()', async () => {
            ...
            await subtest('10a. transfer owner->account1, then verify balances', async () => {
                result.set(await ProxyToken.methods.transfer(account1, tokensToWei(10000)).send({from: owner}));
                result.checkTransactionOk();
                // .
                result.set(await ProxyToken.methods.balanceOf(owner).call());
            ...
            await subtest('10b. make transfer account1->account2, then verify balances', async () => {
                result.set(await ProxyToken.methods.transfer(account2, tokensToWei(1000)).send({from: account1}));
                result.checkTransactionOk();
                // Verify new balances.
                result.set(await ProxyToken.methods.balanceOf(account1).call());
                result.checkIsEqual(tokensToWei(9000));
                result.set(await ProxyToken.methods.balanceOf(account2).call());
                result.checkIsEqual(tokensToWei(1000));
            }).catch(catcher);

            await subtest('10c. trying to transfer more tokens than the balance will fail', async () => {
                result.set(await expectFail(ProxyToken.methods.transfer(account2, tokensToWei(999999)).send({from: account1})).catch(catcher));
                result.checkDidFail();
            ...
            await subtest('10d. smaller transfer succeeds after first failed transfer', async () => {
                result.set(await ProxyToken.methods.transfer(account2, tokensToWei(9000)).send({from: account1}));
                result.checkTransactionOk();
                // Verify new balances.
                result.set(await ProxyToken.methods.balanceOf(account1).call());
                result.checkIsEqual(0);
    ...
    async function approveThenTransferFunds(from, to, amount) {
        ...
        result.set(await ProxyToken.methods.approve(to, amount).send({from: from}));
        result.checkTransactionOk('approveThenTransferFunds(): approve failed');
        // Transfer approved funds 'from' -> 'to'.
        result.set(await ProxyToken.methods.transferFrom(from, to, amount).send({from: to}));
    ...
    if (runThisTest())
        it('11. verified ERC20 transfer/approve method', async () => {
            ...
            await subtest('11a. transfer owner->account1, then verify balances', async () => {
                ...
                result.checkIsEqual(tokensToWei(10000));
                // Verify allowances
                result.set(await ProxyToken.methods.allowance(owner, account1).call());
                result.checkIsEqual(0);
                result.set(await ProxyToken.methods.allowance(account1, owner).call());
                result.checkIsEqual(0);
            }).catch(catcher);
            ...
            await subtest('11b. make transfer account1->account2, then verify balances', async () => {
                ...
                result.checkIsEqual(tokensToWei(1000));
                // Verify allowances
                result.set(await ProxyToken.methods.allowance(account1, account2).call());
                result.checkIsEqual(0);
                result.set(await ProxyToken.methods.allowance(account2, account1).call());
                result.checkIsEqual(0);
            }).catch(catcher);
            ...
            await subtest('11c. trying to transfer more tokens than the balance will fail', async () => {
                ...
                await ProxyToken.methods.approve(account2, 0).send({from: account1});
                // Verify nothing changed.
                result.set(await ProxyToken.methods.balanceOf(account1).call());
                result.checkIsEqual(tokensToWei(9000));
                result.set(await ProxyToken.methods.balanceOf(account2).call());
            ...
            await subtest('11d. smaller transfer succeeds after prior failed transfer', async () => {
                ...
                result.checkIsEqual(tokensToWei(10000));
                // Verify allowances
                result.set(await ProxyToken.methods.allowance(account1, account1).call());
                result.checkIsEqual(0);
                result.set(await ProxyToken.methods.allowance(account2, account1).call());
                result.checkIsEqual(0);
            }).catch(catcher);
    ...
    if (runThisTest())
        it('12. verified transferFrom() fails without prior approval', async () => {
            ...
            await subtest('12a. account1 can\'t transfer from owner to account2', async () => {
                result.set(await expectFail(ProxyToken.methods.transferFrom(owner, account2, tokensToWei(5 * ONE_MILLION)).send({from: account1})).catch(catcher));
                result.checkDidFail();
            }).catch(catcher);

            await subtest('12b. owner can\'t transfer from account1 to account2', async () => {
                result.set(await expectFail(ProxyToken.methods.transferFrom(account1, account2, tokensToWei(5 * ONE_MILLION)).send({from: owner})).catch(catcher));
                result.checkDidFail();
            }).catch(catcher);

            await subtest('12c. owner can\'t transfer from owner to account1', async () => {
                result.set(await expectFail(ProxyToken.methods.transferFrom(owner, account1, tokensToWei(5 * ONE_MILLION)).send({from: owner})).catch(catcher));
                result.checkDidFail();
            }).catch(catcher);

            await subtest('12d. owner can\'t transfer from owner to owner', async () => {
                result.set(await expectFail(ProxyToken.methods.transferFrom(owner, owner, tokensToWei(5 * ONE_MILLION)).send({from: owner})).catch(catcher));
                result.checkDidFail();
            }).catch(catcher);
        });
    ...
    if (runThisTest())
        it('13. burn tokens', async () => {
            ...
            await subtest('13b. non-owner can burn own tokens.', async () => {
                ...
                result.set(await ProxyToken.methods.transfer(account1, tokensToWei(100 * ONE_MILLION)).send({from: owner}));
                result.checkTransactionOk();
                result.set(await ProxyToken.methods.balanceOf(account1).call());
                result.checkIsEqual(tokensToWei(100 * ONE_MILLION));
                // Try to burn 50M from account 1
                result.set(await ProxyToken.methods.burn(tokensToWei(40 * ONE_MILLION)).send({from: account1}));
                result.checkTransactionOk();
            ...
            await subtest('13c. owner can burn own tokens', async () => {
                ...
                result.set(await ProxyToken.methods.burn(tokensToWei(50 * ONE_MILLION)).send({from: owner}));
                result.checkTransactionOk();
                // Confirm tokens are gone
                result.set(await ProxyToken.methods.balanceOf(owner).call());
    ...
    if (runThisTest())
        it('20. test today()', async () => {
            ...
            await subtest('20a. theck today() computation', async () => {
                result.set(await ProxyToken.methods.today().call());
    ...
    async function testGrantVestingTokens_balances(owner, account, [totalAmount, vestingAmount, startDay], [duration, cliffDuration, interval, isRevocable]) {
        ...
        const ownerBalanceBefore = await ProxyToken.methods.balanceOf(owner).call();
        const accountBalanceBefore = await ProxyToken.methods.balanceOf(account).call();
        ...
        const ownerBalance = await ProxyToken.methods.balanceOf(owner).call();
        ...
        let accountBalance = await ProxyToken.methods.balanceOf(account).call();
        ...
        accountBalance = await ProxyToken.methods.vestingAsOf(Math.floor(startDay)).call({from: account, gas: VESTASOFGAS});
    ...
    async function testVestingScheduleDayByDay(account, [totalAmount, vestingAmount, startDay], [duration, cliffDuration, interval, isRevocable = false]) {
        ...
        for (let elapsedDays = -dayIncrement; elapsedDays <= (duration + dayIncrement); elapsedDays += dayIncrement) {
            ...
            result.set(await ProxyToken.methods.vestingAsOf(Math.floor(onDay)).call({from: account, gas: VESTASOFGAS}));
            checkResult('vestingAsOf() ');

            // Test using the grant administrator access method
            result.set(await ProxyToken.methods.vestingForAccountAsOf(account, Math.floor(onDay)).call({from: account}));
    ...
    async function testAccountHasNoGrant(account, wasRevoked, onDay) {
        result.set(await ProxyToken.methods.vestingAsOf(onDay).call({from: account, gas: VESTASOFGAS}));
    ...
    if (runThisTest())
        it('21. test grantVestingTokens() success cases', async () => {
            ...
            result.set(await ProxyToken.methods.registerAccount().send({from: account9}));
            result.checkTransactionOk();
            result.set(await ProxyToken.methods.safeTransfer(account9, tokensToWei(100 * ONE_MILLION)).send({from: owner}));
            result.checkTransactionOk();
            result.set(await ProxyToken.methods.transferOwnership(account9).send({from: owner, gas: XFEROWNERGAS}));
            result.checkTransactionOk();
    ...
    if (runThisTest())
        it('23. test grantVestingTokens() failure cases (account limits, ownership transfer)', async () => {
            ...
            await ProxyToken.methods.registerAccount().send({from: newOwner});
            await ProxyToken.methods.safeTransfer(newOwner, tokensToWei(100 * ONE_MILLION)).send({from: owner});
            await ProxyToken.methods.transferOwnership(newOwner).send({from: owner, gas: XFEROWNERGAS});
    ...
    if (runThisTest())
        it('24. test usage of granted tokens (contract use of today\'s date)', async () => {
            ...
            await subtest('24a. verified granted tokens outside of vesting can be transferred before vesting begins', async () => {
                ...
                result.set(await expectFail(ProxyToken.methods.transfer(account2, tokensToWei(1)).send({from: account1})).catch(catcher));
                result.checkDidFail();
            }).catch(catcher);
            ...
            await subtest('24a. verified no tokens can be transferred with an unvested grant', async () => {
                ...
                result.set(await ProxyToken.methods.transfer(account4, tokensToWei(1)).send({from: account3}));
                result.checkTransactionOk('Transfer of 1 token from grant was supposed to work');

                // Make sure a too-large transfer fails.
                result.set(await expectFail(ProxyToken.methods.transfer(account4, tokensToWei(2)).send({from: account3})).catch(catcher));
                result.checkDidFail();
            }).catch(catcher);
        });
    ...
    async function testRevokeVestingSchedule(owner, account, [totalAmount, vestingAmount, startDay], [duration, cliffDuration, interval, isRevocable], revokeOnDay) {
        ...
        result.set(await ProxyToken.methods.vestingAsOf(revokeOnDay).call({from: account, gas: VESTASOFGAS}));
        const vestedAsOfDay = result.value()[0];
        const notVestedAsOfDay = result.value()[1];

        const ownerBalanceAfterGrant = await ProxyToken.methods.balanceOf(owner).call();
        const accountBalanceAfterGrant = await ProxyToken.methods.balanceOf(account).call();
        ...
        result.set(await ProxyToken.methods.revokeGrant(account, revokeOnDay).send({from: owner, gas: REVOKEONGAS}));
        result.checkTransactionOk('revokeGrant() failed');

        // Verify token balances (tokens should NOT have moved)
        const ownerBalanceAfterRevocation = await ProxyToken.methods.balanceOf(owner).call();
        const accountBalanceRevocation = await ProxyToken.methods.balanceOf(account).call();
        ...
        result.set(await ProxyToken.methods.allowance(owner, account).call());
        result.checkIsEqual(0, 'There should be no leftover allowance in the correct direction');
        result.set(await ProxyToken.methods.allowance(account, owner).call());
    ...
    if (runThisTest())
        it('25. test date ranges and grant duration()', async () => {
            ...
            await subtest('25a. First, verify this schedule is allowed on a normal date', async () => {
                ...
                result.set(await ProxyToken.methods.revokeGrant(account1, TODAY_DAYS).send({from: owner, gas: REVOKEONGAS}));
                result.checkTransactionOk('revokeGrant() failed');
            }).catch(catcher);
    ...
    async function testRoleBehavior(testNumber, roleName, isRoleMethod, addRoleMethod, addRoleParams, removeRoleMethod) {
        result.set(await ProxyToken.methods.isOwner().call({from: owner}));
        result.checkIsTrue('Expected original owner to be owner');
        result.set(await ProxyToken.methods[isRoleMethod](owner).call({from: owner}));
        ...
        await subtest(testNumber + 'a. transfer contract ownership to account 1', async () => {
            ...
            result.set(await ProxyToken.methods.owner().call({from: owner}));
            result.checkIsEqual(account1);
        }).catch(catcher);
        ...
        await subtest(testNumber + 'c. Check that new owner is now both owner and has role ' + roleName, async () => {
            ...
            result.set(await ProxyToken.methods[isRoleMethod](account1).call({from: account1}));
        ...
        await subtest(testNumber + 'd. test remove role ' + roleName + ' from owner account1, leaving no other account having role (specifically allowed!)', async () => {
            result.set(await ProxyToken.methods[removeRoleMethod](account1).send({from: account1}));
        ...
        await subtest(testNumber + 'e. test addRoleMethod() success cases', async () => {
            result.set(await ProxyToken.methods[addRoleMethod](account5, ...addRoleParams).send({from: account1}));
            ...
            result.set(await ProxyToken.methods[removeRoleMethod](account5).send({from: account1}));
        ...
        await subtest(testNumber + 'f. test addRoleMethod() failure cases', async () => {
            ...
            result.set(await expectFail(ProxyToken.methods[addRoleMethod](account3, ...addRoleParams).send({from: owner})).catch(catcher));
            result.checkDidFail();
            result.set(await ProxyToken.methods[isRoleMethod](account3).call({from: owner}));
            ...
            result.checkIsFalse("account3 should start off not having role " + roleName);
            result.set(await expectFail(ProxyToken.methods[addRoleMethod](account3, ...addRoleParams).send({from: account6})).catch(catcher));
            result.checkDidFail();
            result.set(await ProxyToken.methods[isRoleMethod](account3).call({from: account6}));
        ...
        await subtest(testNumber + 'g. non-owner (without the role) trying to add/remove role' + roleName, async () => {
            result.set(await expectFail(ProxyToken.methods[addRoleMethod](account4, ...addRoleParams).send({from: account3})).catch(catcher));
            result.checkDidFail();
            result.set(await expectFail(ProxyToken.methods[removeRoleMethod](account4).send({from: account3})).catch(catcher));
        ...
        await subtest(testNumber + 'h. non-owner should not be able to add/remove role' + roleName, async () => {
            ...
            result.set(await expectFail(ProxyToken.methods[removeRoleMethod](account4).send({from: account3})).catch(catcher));
            result.checkDidFail('account3 should NOT have been able to remove role ' + roleName + ' from other account');
            result.set(await expectFail(ProxyToken.methods[removeRoleMethod](account1).send({from: account3})).catch(catcher));
        ...
        await subtest(testNumber + 'i. non-owner (having the role) should not be able to add/remove role' + roleName, async () => {
            ...
            result.set(await expectFail(ProxyToken.methods[addRoleMethod](account6, ...addRoleParams).send({from: account5})).catch(catcher));
            result.checkDidFail('Account with role ' + roleName + ' should NOT have been able to add same role.');
            result.set(await expectFail(ProxyToken.methods[removeRoleMethod](account6).send({from: account5})).catch(catcher));
    ...
    if (runThisTest())
        it('32. test that pausable ERC20 functionality is blocked while paused', async () => {
            ...
            result.set(await ProxyToken.methods.transfer(account1, tokensToWei(100 * ONE_MILLION)).send({from: owner}));
            result.checkTransactionOk();
            result.set(await ProxyToken.methods.transfer(account2, tokensToWei(100 * ONE_MILLION)).send({from: owner}));
            result.checkTransactionOk();

            await subtest('32a. try a valid call to transfer(), paused and not paused', async () => {
                await testPausableMethod(
                    ProxyToken.methods.transfer,
            ...
            await subtest('32d. try a valid call to increaseAllowance(), paused and not paused', async () => {
                await testPausableMethod(
                    ProxyToken.methods.increaseAllowance,
            ...
            await subtest('32e. try a valid call to decreaseAllowance(), paused and not paused', async () => {
                await testPausableMethod(
                    ProxyToken.methods.decreaseAllowance,
                    [account5, tokensToWei(ONE_MILLION)],
                    account2).catch(catcher);
            }).catch(catcher);
        });
    ...
    if (runThisTest())
        it('40. test uniform grantor setup', async () => {
            ...
            await subtest('40h. check that owner CAN set restrictions and vesting schedule on registered grantor.', async () => {
                ...
                result.checkTransactionOk();
                result.set(await ProxyToken.methods.setGrantorVestingSchedule(grantor, ...vestingSchedule).send({from: owner}));
                result.checkTransactionOk();
            }).catch(catcher);
            ...
            await ProxyToken.methods.safeTransfer(grantor, tokensToWei(100 * ONE_MILLION)).send({from: owner});
